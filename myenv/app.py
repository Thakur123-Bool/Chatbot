...
def process_pdf(pdf_files: List[UploadFile]):
    if not pdf_files:
        return None, None, None  # Return None if no PDFs are uploaded

    all_chunks = []
    for pdf in pdf_files:
        pdf_bytes = pdf.file.read()  # Read the file content into bytes
        loader = PyMuPDFLoader(BytesIO(pdf_bytes))  # Use BytesIO to load PDF from bytes
        data = loader.load()
